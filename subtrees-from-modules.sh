...
if [[ $i == \[submodule* ]]; then
    ...
    mcommit=$(git submodule status $mpath | grep -E "\S+" -o | head -1 | cut -d"-" -f2)
    ...
    # add the remote
    git remote add -f $mname $PWD/.git/modules/$mpath #//$murl
    ...
    # fetch the files
    git fetch $PWD/.git/modules/$mpath 
...
#
#set(gRPC_INSTALL ON CACHE BOOLEAN "" )
#set(gRPC_BUILD_TESTS OFF CACHE BOOLEAN "" )
#set(CMAKE_BUILD_TYPE Release CACHE STRING "")
#set(ABSL_ENABLE_INSTALL ON CACHE BOOLEAN "" )
